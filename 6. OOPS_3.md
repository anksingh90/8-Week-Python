...
Q : Implement abstract class in python.
Accept charges from user and process according to the method invoked.

- Create an `abc` class - `"PaymentGateway"` which holds respective methods : 
- **abc should act as blueprints(only pass the method) without any implementation body.**
	- **charge(self, amount)** : abstractmethod
	- **refund(self, amount)** : abstractmethod
	- **receipt(self, amount)**
		- returns - "Receipt: ₹[amount] processed via [Class Name : CreditCard/UPI]"

- **Subclass Implementation:**
	- **class CreditCard(PaymentGateway):**
		- Holds 2 methods : **charge(self, amount)** & **refund(self, amount)**
		- both returns - `"Credit Card charged/refund ₹{amount}"`
	- **class UPI(PaymentGateway):**
		- Holds 2 methods : **charge(self, amount)** & **refund(self, amount)**
		- both returns - `"UPI charged/refund ₹{amount}"`

- **Instances of class :**  
	- 2 separate object for both classes.
	- Call the `charge()` method on the credit card object (e.g., with ₹500).
	- Call the `refund()` method on the UPI object (e.g., with ₹200).
	- Call the inherited `receipt()` method on the credit card object (e.g., with ₹500).

Required Output : 
```text
Credit Card charged ₹500
UPI refund of ₹200 initiated
Receipt: ₹500 processed via CreditCard
```
<!--
```python
from abc import ABC, abstractmethod
...
**Def** : It is feature in Python that allows you to change or customize how built-in operators (like +, -, *, ==) behave when used with your own user-defined objects.
...
class Vector:
    def __init__(self, x, y):
        self.x = x				# assigning values from object
        self.y = y

    def __add__(self, other):		# Operator Overloading for the addition (+)
		# self accepts v1 value and other accepts v2 value automatically.
		return Vector(self.x + other.x, self.y + other.y)

    def __repr__(self):				# String Representation
		# Without this method, v1 + v2 would output a generic response : <__main__.Vector object at 0x7f9...>
...
# python secretly translates it to v1.__add__(v2)
...
| Method | Trigger Syntax | Action / Purpose |
|---|---|---|
| `__contains__(self, item)` | `item in obj` | Checks if an item exists in the container; returns `True` or `False`. |
| `__getitem__(self, key)` | `obj[key]` | Retrieves the value associated with the specified key or index. |
| `__setitem__(self, key, value)` | `obj[key] = value` | Assigns or updates a value for the specified key or index. |
